...
At this point, you might be wondering whether you can modify the color that the SpinWheel pulses in the above sketch. By modifying the line `SpinWheel.setLargeLEDsUniform(b, b, b)`, you can do just that! For instance, to make the SpinWheel be a teal color instead of white, instead change that line to be `SpinWheel.setLargeLEDsUniform(0, b, b)`. What about making the LEDs light up yellow? To experiment more quickly, first try this in the virtual SpinWheel above. Then when you have some modifications that you are interested in trying out on your SpinWheel, you can modify the example sketch that you opened in the same way.
...
then the code for the virtual SpinWheel. 
